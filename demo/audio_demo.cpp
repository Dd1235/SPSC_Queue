...
static void write_wav(const std::string& path, const std::vector<std::int16_t>& s) {
    ...
    std::fwrite("RIFF", 1, 4, f); put_le(f, 36 + dataBytes, 4); std::fwrite("WAVE", 1, 4, f);
    ...
    std::fwrite("data", 1, 4, f); put_le(f, dataBytes, 4);
...
static Result run(const char* wavPath, int numBuffers) {
    Channel ch(1024);
    ...
    std::thread control([&] {
        ...
        while (!done.load(std::memory_order_relaxed)) {
            ...
            double busy = 0.0;
            if (t >= nextStallMs) { busy = stallDist(rng); nextStallMs += kStallEveryMs; }
    ...
    r.p99DrainMs = drain.empty() ? 0 : drain[static_cast<std::size_t>(0.99 * (drain.size() - 1))];
    return r;
}
...
int main(int argc, char** argv) {
    ...
    std::printf("  %u Hz, %d-frame buffers => %.2f ms budget per callback\n",
                kSampleRate, kBufferFrames, kBufferMs);
    ...
    std::printf("  busy %.0f-%.0f ms every ~%.0f ms (priority inversion)\n",
                kStallMinMs, kStallMaxMs, kStallEveryMs);
    ...
    std::printf("%-26s %12.0f ms %14.0f ms\n", "  = silence/glitch time",
                mx.xruns * kBufferMs, lf.xruns * kBufferMs);
    std::printf("%-26s %12.3f ms %14.3f ms\n", "max wait for data", mx.maxDrainMs, lf.maxDrainMs);
    std::printf("%-26s %12.3f ms %14.3f ms\n", "p99 wait for data", mx.p99DrainMs, lf.p99DrainMs);
